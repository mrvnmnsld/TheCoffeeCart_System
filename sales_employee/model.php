<?php
    ...
    if($_POST["action"]=="getDailySales"){
        $column = array('reciept_no','username','firstname','lastname','discount','TotalPrice','date');

	$query = "SELECT sales_product.reciept_no,sales.discount, sales.total as TotalPrice,username,date,customer.firstname,customer.lastname FROM sales_product JOIN sales ON sales_product.reciept_no=sales.reciept_no JOIN customer ON sales.customer_id = customer.firstname WHERE date LIKE '%".date('Y-m-d')."%'";
